# 纸鸢求职助手

纸鸢求职助手围绕求职机会发现、JD 评估、简历优化、面试准备和投递追踪组织用户的求职工作流。这个词表用于固定产品语言，避免同一能力在界面、文档和 Agent 工具里被不同名称反复表达。

## Language

**Agent Conversation**:
用户与纸鸢持续交互的对话容器；它拥有自己的 Conversation Turn 和 Agent Run，一个 Run 可以引用同一 Conversation 中多个 Turn。切换对话不改变旧 Run 的执行，也不把旧 Run 的状态带入新对话。
_Avoid_: Agent Run, 聊天页面, Session 状态

**Conversation Turn**:
Conversation 中一次用户输入及其对应交互的稳定记录；它可以发起新的 Agent Run，也可以为等待中的 Run 提供补充信息或批准。
_Avoid_: Agent Run, API 请求, 模型轮次

**JD 简历匹配策略**:
一份 JD 及其重做分析、后续追问是否对照用户简历的约束。默认匹配；用户明确要求不匹配时，仅对这份 JD 生效，新 JD 恢复默认匹配。
_Avoid_: 对话级简历开关, 永久求职偏好

**简历诊断**:
对一份简历本身的内容、结构、ATS 可读性、证据不足处和改进方向进行只读评估；没有 JD 也能完成，有 JD 时可附加岗位匹配，不自动修改简历。
_Avoid_: 简历查询, 简历修改, JD 评估

**Run Admission**:
将 Conversation Turn、当前 Agent Run 与引用 Artifact 解析为继续当前 Run、在当前 Conversation 创建新 Run、请求澄清或拒绝的权威决定；它同时固定目标、约束与材料关系。
_Avoid_: 前端路由, Agent 选择, newSession 参数, Task 分类

**岗位发现**:
从用户的目标方向、关键词、排除词、城市和公司来源中持续发现可处理的岗位机会，并把结果沉淀为可查看、可保存、可评估、可跳过的机会池。
_Avoid_: 职位搜索, 岗位搜索, 搜职位

**岗位发现工作台**:
岗位发现的可视化管理界面，用于查看扫描历史、管理岗位机会、处理失败来源，并提供高级扫描配置入口。它不是岗位发现的唯一入口；对话式发起任务属于 Agent Chat。
_Avoid_: 职位搜索页, 搜索首页, 管理界面

**岗位发现结果卡**:
Agent Chat 中呈现单个岗位机会的结构化卡片，承载岗位摘要、原 JD 链接、打开 JD、保存、评估和跳过等核心动作。它默认只展示本轮前 5 个核心结果，更多结果进入岗位发现工作台管理。
_Avoid_: 搜索结果, 文本列表, JD 列表项

**岗位发现确认**:
Agent Chat 在真正创建岗位发现任务前呈现的轻量确认步骤，用于让用户确认关键词、排除词、城市、结果上限和来源范围。它保护会写入机会池的扫描任务，不用于纯只读查询。
_Avoid_: 权限弹窗, 二次确认弹窗, 搜索确认

**岗位机会池**:
岗位发现沉淀出的候选机会集合，包含岗位标题、公司、原链接、摘要、状态和可能的错误信息。岗位机会池不是 JD 库；只有用户明确保存或评估的岗位才进入 JD 库。
_Avoid_: JD 库, 职位列表, 搜索缓存

**换一批**:
在岗位发现语境中，优先表示从当前扫描或岗位机会池中展示下一批未展示机会，而不是立即启动新的扫描任务。只有机会池没有更多结果时，才引导用户放宽条件并重新扫描。
_Avoid_: 重新扫描, 重新搜索

**画像预填**:
Agent 在岗位发现确认前，用用户求职画像中的目标岗位、城市偏好或底线条件预填扫描条件。画像预填必须在确认卡里明示，不能隐式替用户启动会写入机会池的任务。
_Avoid_: 自动搜索, 静默推荐, 隐式扫描

**明确求职偏好**:
用户本人向系统直接表达的当前求职目标或约束，可由用户后续纠正；它不同于简历字段、历史经历、引用材料、假设和模型推断。
_Avoid_: 简历字段, 旧画像, 引用或假设, 推断偏好

**持续岗位发现**:
未来按用户确认的目标方向和频率持续创建岗位发现任务的能力，用于定期补充岗位机会池。它不属于第一版对话式岗位发现，但第一版命名和数据模型不能把岗位发现限制成一次性搜索。
_Avoid_: 定时搜索, 自动海搜, 自动投递

**岗位指纹**:
用于判断岗位机会是否重复的稳定标识。第一版以规范化后的原 JD 链接作为强岗位指纹；公司、岗位标题和城市只能作为可能重复提示，不能直接阻止入库。
_Avoid_: 原始 URL, 搜索结果 ID, 卡片 ID

**简历文档**:
用户在纸鸢中维护的结构化简历及其版本、原文来源、内容哈希和读回状态；服务端数据是唯一事实源，浏览器缓存只是投影。
_Avoid_: 简历文本, CV 缓存, 页面状态

**简历摄取**:
将用户上传或粘贴的简历材料抽取、分栏、校验并创建新简历版本的过程。摄取结果不能在只更新页面状态后结束，必须完成服务端持久化和读回。
_Avoid_: 简历解析, 导入结果, OCR 结果

**原文工件**:
简历摄取时保存的原始文件、完整抽取文本及其来源诊断，用于追溯、重解析和证明结构化内容没有静默丢失。
_Avoid_: 附件, 临时文本, 上传缓存

**完整性证据**:
用于证明原文片段、实体、日期、数字和结构化 section 之间覆盖关系的可机器检查记录。
_Avoid_: 解析成功, 非空校验, 模型自检

**简历读取投影**:
从简历文档生成的不同用途视图：用户可查看的完整 section、Agent 按 section/chunk 读取的上下文，以及工具卡片使用的结构化数据。
_Avoid_: 截断文本, 工具结果, 简历摘要

**简历修改事务**:
由一个或多个 section patch 组成、绑定基础版本和哈希、可审批、可原子应用、可读回验证和可回滚的简历变更。
_Avoid_: 聊天里的改写文本, 保存按钮, 优化结果

**导入版本**:
一次简历摄取生成的完整候选版本，包含结构化内容、原文工件和完整性状态；只有通过验证或用户明确确认后才成为活动版本。
_Avoid_: 临时解析结果, 上传内容, 覆盖版本

**简历草稿工件**:
由 Agent 生成并持久化的一个或多个简历修改方案，拥有稳定的 artifact id、variant id、section patch 和完整性证据；聊天消息只引用它，不承载其唯一正文。
_Avoid_: 方案文本, 最近一条消息, 优化回复

**Agent Run**:
为完成一个用户目标而创建的持久执行实体；它拥有独立身份和生命周期，不因浏览器关闭、连接中断或执行进程重启而变成另一项任务。
_Avoid_: Run 台账, API 请求, 聊天会话, 页面任务

**运行续跑**:
Agent Run 中断后，从已持久化的安全执行位置继续同一个 Run；它保留原 Run 身份，不等同于重试一次请求或重新加载聊天记录。
_Avoid_: 会话恢复, 页面恢复, 重新执行, Retry

**Continuation Stimulus**:
能够推动等待或暂停中的 Agent Run 继续执行的有序持久输入，包括 Conversation Turn、Gate 决策、后台作业结果和人工恢复指令；记录、消费与执行完成是三个不同事实。
_Avoid_: 回调, 唤醒信号, 临时事件, 已消费输入

**等待用户**:
Agent Run 尚未结束但必须取得用户信息或批准才能安全继续的状态；用户后续输入会继续同一个 Run，而不是创建替代任务。
_Avoid_: 执行失败, Run 结束, 页面暂停

**Run Gate**:
Agent Run 继续执行前必须满足的一项持久条件，例如取得缺失信息或批准一个精确工具动作；Gate 的范围和状态属于 Run。
_Avoid_: 权限弹窗, Tool Governance 状态, 永久授权

**Recovery Attempt**:
Recovery Supervisor 在 Agent Run 内为跨过一个已分类失败而选择的一次有预算恢复动作；它保留在同一 Run 中，并受已有尝试与风险约束。
_Avoid_: Repair, 无限重试, 工程修复

**工程修复建议**:
Run 结束后由 Review 从证据中提炼的代码、配置或测试改进方向；它不能改变已经结束的 Run 结果。
_Avoid_: Recovery Attempt, 自动修复, repairing 状态

**Run Contract**:
绑定一个 Agent Run 的完成约束，定义该目标允许使用的能力、需要的批准、验证要求以及什么结果可以被称为成功。
_Avoid_: Task Contract, 提示词规则, 工具白名单

**Task Program**:
一类用户目标在 Agent Run 内允许经历的阶段、可使用的能力、必须形成的验证事实和成功出口；它规定执行骨架，模型只生成当前阶段所需内容。
_Avoid_: Prompt, Tool 列表, Run Contract, 模型计划

**确定性 Program**:
必须按已定义阶段完成副作用、读回验证或 Artifact 持久化的 Task Program，任何必做阶段缺失都不能声明成功。
_Avoid_: 高风险 Prompt, 事后成功判分, Tool 白名单

**对话型 Program**:
以只读回答或引导为目标、保留上下文绑定和成功约束但不强制写入或 Artifact 阶段的 Task Program。
_Avoid_: 无 Contract 对话, 自由聊天, 确定性 Program

**模拟面试主问题**:
一场模拟面试中独立考查一个能力点的题目，是面试进度的计数单位；追问和反问练习不属于主问题。
_Avoid_: 追问, 反问练习, 一条聊天消息

**模拟面试追问**:
针对当前主问题的回答补足具体行为、判断依据或结果证据的附属问题；它始终归属原主问题，不形成多层追问链。
_Avoid_: 新主问题, 嵌套追问, 换话题

**模拟面试反问练习**:
候选人在面试结尾向模拟面试官练习提问的独立环节，不计入面试官的主问题或追问。
_Avoid_: 主问题, 面试官追问

**Run Context**:
Runtime 从 Conversation Turn、Run Contract、计划进度、Gate、已完成 Tool Attempt 和事实源引用确定性构建的模型上下文；浏览器消息列表不是它的事实源。
_Avoid_: 前端 messages, Prompt 拼接, 聊天缓存

**Plan Item**:
Agent Run 为推进目标形成的一个有序工作项；它不是独立的用户任务，也不拥有脱离所属 Run 的完成状态。
_Avoid_: Task, 子任务, Step

**Tool Attempt**:
Agent Run 对一个具体工具动作的一次受治理执行尝试；同一动作在恢复过程中可能产生多个可区分的 Tool Attempt。
_Avoid_: Tool Call, 工具结果, Agent Step

**后台作业**:
由长耗时工具创建并独立持久化的工作实体；Tool Attempt 通过稳定标识引用它，Agent Run 可以在其完成、失败或需要处理时续跑。
_Avoid_: 长请求, 后台 Promise, detached 进程

**Run Evidence**:
Agent Run 及其 Tool Attempt 产生的可审计事实，用于向用户展示进度并支持治理、Review 和 Eval；它不是 Run 执行状态本身。
_Avoid_: 日志, Run 台账, 调试信息

**过程状态轨道**:
Agent Run 在执行、等待、校验或恢复期间面向用户展示的受控进度视图；它只表达可理解的阶段和状态，不承载系统提示词、Skill 正文、工具参数或原始推理。
_Avoid_: 思维链, 加载文案, 工具日志

**推理摘要**:
经过策略约束、面向用户改写的 Agent 工作说明，用于解释当前正在比较、查找或核对什么；它不是模型原始推理，也不等同于完整 Run Evidence。
_Avoid_: Chain of Thought, 原始思考, 系统提示词

**安全工具视图**:
从 Tool Attempt 结果中筛选出的、允许进入用户界面的摘要或结构化结果；没有安全工具视图的内部工具结果默认静默或显示统一降级状态。
_Avoid_: 原始工具结果, 工具 JSON, Skill 内容

**合法任务转换图**:
定义一个 Agent Run 或 Conversation 中哪些任务类型可以衔接、可传递哪些 Artifact、是否需要确认以及旧 Run 如何处理的产品规则集合。
_Avoid_: 任意任务跳转, 模型自由切换

**组合求职旅程**:
由多个合法任务转换组成、用于验证跨能力衔接和中断恢复的完整用户目标路径，例如 JD 分析到简历诊断再到面试准备。
_Avoid_: 测试脚本, 多轮聊天样例

**Agent 安全视图**:
Agent Conversation 中允许普通用户看到的文本、状态、确认节点或结构化 Artifact 视图；每类任务都必须定义安全视图，但不要求都表现为复杂卡片。
_Avoid_: 通用工具结果卡, 原始事件, Debug 数据

**Conversation Item**:
由 Durable Run Event、Artifact 和 Gate 生命周期投影出的稳定有序用户界面记录；文本、过程状态、工具视图、Artifact 卡片和确认节点是不同的 Item 类型。
_Avoid_: Session Message, 原始 Run Event, 页面临时状态, Tool Result

**Eval Run**:
一次有版本标识的评测执行记录，绑定代码、模型、提示词、工具、fixture 和 Judge 版本，并保存门禁结果、质量评分及失败证据。
_Avoid_: CI 日志, 测试报告文件, Agent Run

**评测样本**:
可复跑的任务输入、程序化事实、期望约束和脱敏元数据集合；它可以来自模板合成、脱敏真实案例或线上失败晋升。
_Avoid_: Prompt 示例, Mock 返回值, 聊天截图

**质量 Judge**:
使用任务专属 rubric 对 Agent 产物质量评分的独立评测模型；它不能覆盖权限、归属、读回或协议安全等确定性失败。
_Avoid_: 生成模型自评, 唯一发布门禁, 人工审批

**任务切换**:
用户在当前 Agent Run 未完成时明确发起另一目标；到达安全切换点后，旧 Run 进入可恢复暂停状态，并在同一 Conversation 创建新 Run。补充当前目标的信息不属于任务切换。
_Avoid_: 取消任务, 新消息, Agent 切换

**安全切换点**:
Agent Run 可以暂停且不会遗留结果不确定的 Tool Attempt 或未处理高风险 Gate 的持久位置；未到达该位置时，新目标必须等待当前动作完成、拒绝或取消。
_Avoid_: 任意 checkpoint, 浏览器空闲, 模型输出结束

**主责交接**:
主责 agent 将执行责任交给另一专业 agent 的受治理动作；目标与 Program 不变时保留同一 Run，跨目标时沿合法任务转换创建新 Run 并传递允许的 Artifact 引用。
_Avoid_: Agent 切换, 自由跳转, 模型间私聊

**研究委派**:
主责 agent 为获取只读研究结果而创建挂靠当前 Run 的子 Agent Run；子 Run 不执行任何写操作，完成时向主责 agent 返回结构化摘要，不向用户展开完整子对话。
_Avoid_: 子任务, 子 agent 泛称, 无限嵌套对话

**研究委派依赖级别**:
主 Run 对子 Run 研究结果的依赖，分为缺失时不能完成目标的必需研究与可说明缺失后继续的辅助研究；依赖级别由目标 Program 确定，不由执行中的模型临时放宽。
_Avoid_: 子 agent 重要性, 模型自评, 工具优先级

**记忆事实账本**:
长期记忆中以双时间轴记录用户相关事实的账本；事实带有生效与失效时间，被新事实取代时作废而非删除，每条事实可回溯到原始信号。
_Avoid_: 记忆条目泛称, 向量库, 覆盖更新

**个人记忆候选**:
从用户闲聊中提取、尚待用户确认的个人事实提案；它不参与个性化推荐或任务决策，且与团队共享材料的审核候选不同。
_Avoid_: 活跃记忆, 团队审核候选, 用户明确偏好

**记忆纠正**:
用户确认某条活跃记忆有误并给出替代事实的操作；旧事实失效、新事实带来源生效，历史证据保留。
_Avoid_: 记忆清除, 覆盖原文, 模型自行改写

**记忆删除请求**:
用户当前通过对话或记忆管理入口明确要求删除指定记忆内容的操作意图；对自身遗忘的叙述、引语、假设或含糊的“忘记”表述均不构成该请求。
_Avoid_: 事实过时, 偏好纠正, 记忆提取

**记忆清除**:
对已确认的指定记忆及其记忆层派生副本执行清除，并阻止保留的原始材料将其自动重新提取为记忆；它不等同于事实因新证据而失效，也不自动删除简历、报告或聊天原文。
_Avoid_: 事实失效, 删除账户, 删除原始材料, 隐藏检索结果

**记忆分区**:
按读写权限划分的长期记忆命名空间；每个分区声明可读与可写范围，多个 Agent 的共享与隔离统一由分区权限表达。
_Avoid_: 任务白名单, 全局记忆, memory-policy 规则

**私人记忆**:
仅服务于所属用户的个人事实与会话观察；用户本人可以查看、纠正或请求删除，管理员默认只能看到脱敏治理指标。
_Avoid_: 团队共享记忆, 管理员默认可见, 公开画像

**会话观察记忆**:
Conversation 内部的观察与反思两级记录：观察是带时间戳的具体事件，反思是从观察沉淀出的事实；它服务长对话的连续性，经由写入决策才能进入长期记忆。
_Avoid_: 聊天记录, 会话摘要, 长期记忆
