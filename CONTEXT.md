...
**主责交接**:
主责 agent 沿合法任务转换图的边将 Agent Run 的主责转移给目标 agent 的动作；交接必须携带结构化交接元数据和需转发的 Artifact 引用，图外交接必须取得用户确认。
...
**澄清 Run**:
意图不足以确定任务时创建的过渡 Agent Run：以通用对话任务承载，提出一个精确追问后进入等待用户；用户答复后判出真实任务时，澄清 Run 完结并沿合法任务转换图创建新 Run。
_Avoid_: 澄清弹窗, 意图拒绝, 无 Run 的 202 响应

**双面画布**:
纸鸢界面的双栏形态：对话面恒定承载伙伴对话与需要用户决策的卡片；分析面按需展开承载报告、提案预览与岗位池等证据内容，最大化即为工作台。
_Avoid_: 单列聊天流, 弹窗报告, 浮层工作台
