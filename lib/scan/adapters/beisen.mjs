...
export async function fetchJobsPlaywright(company, page) {
  ...
  const limits = { max_jobs: 200, max_scrolls: 50, scroll_wait_ms: 2000, ...(company.limits || {}) };
  ...
  while (pageNum < limits.max_scrolls && allJobs.length < limits.max_jobs) {
    await page.waitForTimeout(limits.scroll_wait_ms);
