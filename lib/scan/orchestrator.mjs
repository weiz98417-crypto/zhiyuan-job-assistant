 ...
 * @returns {{ scanId: string, conflict: boolean }}
 */
 ...
 * @param {string} scanId
 */
export function getScanStatus(db, scanId) {
  const scan = db.prepare("SELECT * FROM scan_queue WHERE id = ?").get(scanId);
  if (!scan) return null;
  ...
  const errorLog = JSON.parse(scan.error_log || '[]');
  const companyStatus = companies.map((/** @type {any} */ c) => ({
    name: c.company,
    status: errorLog.find((/** @type {any} */ e) => e.company === c.company) ? 'error' : 'success',
    jobsFound: c.jobs_found,
    error: errorLog.find((/** @type {any} */ e) => e.company === c.company)?.error || null,
  }));
...
export function getActiveScan(db, userId) {
  ...
  if (!scan) return null;
  return getScanStatus(db, scan.id);
}
...
export function getScanHistory(db, userId, opts = {}) {
  ...
  return {
    history: scans.map((/** @type {any} */ s) => ({
      ...
      failedCompanies: JSON.parse(s.error_log || '[]'),
    })),
    total: total.count,
    page: opts.page || 1,
  };
 ...
 */
export function resetCompanyFlag(db, scanId, company) {
  const scan = db.prepare("SELECT error_log FROM scan_queue WHERE id = ?").get(scanId);
