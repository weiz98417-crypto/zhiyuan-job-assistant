...
import { filterRoleQuality, matchesExpandedJob } from './query-expansion.mjs';
...
export function applyTitleFilter(jobs, titleFilter) {
  ...
  return filterRoleQuality(jobs.filter(job => matchesExpandedJob(job, titleFilter)));
}
...
export function getScanStatus(db, scanId, userId) {
  ...
  const companies = db.prepare(`
    SELECT COALESCE(NULLIF(source_name, ''), company) AS company, COUNT(*) as jobs_found
  ...
  const sourceRuns = db.prepare(`
    ...
    FROM scan_source_runs WHERE scan_id = ?
    ORDER BY id ASC
  `).all(scanId);
  ...
  const sourceStatus = sourceRuns.map((/** @type {any} */ run) => ({
    name: run.source_name,
    sourceType: run.source_type,
    status: run.status,
    jobsFound: run.inserted || 0,
    ...
    metrics: (() => { try { return JSON.parse(run.metrics_json || '{}'); } catch { return {}; } })(),
  }));

  const companyStatus = sourceStatus.length ? sourceStatus : companies.map((/** @type {any} */ c) => ({
    name: c.company,
