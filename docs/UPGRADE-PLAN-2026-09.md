...
- **长期层 = 自研 Postgres 事实账本**：bi-temporal 三表（episodes 通常保留历史 / entities 演化摘要 / facts 带 valid_at·invalid_at·source_episode_id），矛盾时作废而非覆盖，已确认的定向记忆清除按 ADR-0033 跨层删除；结构化求职画像（目标岗位/城市偏好/底线/黑名单等 topic/sub_topic 字段集，纯 SQL 不走向量）；记忆分区（MemCube 式命名空间 + 显式读写列表，取代任务型 memory-policy）。私人记忆由用户查看、纠正与删除；admin 只治理团队共享候选和脱敏指标，个案访问须用户限时授权并审计。
- **写入管线**：确定性完成事件即时写（JD 评估完成、简历保存成功等带证据事实）+ 对话信号后台批量 flush（闲置/token 阈值）形成待确认候选；闲聊个人事实经用户确认才进入活跃记忆。提取升级为 Mem0 两段式（抽取候选 → 与相似旧事实比对 → ADD/UPDATE/DELETE/NOOP + 理由落库），其中 `DELETE` 仅使事实失效，不能冒充清除。用户明确表达的当前偏好按 ADR-0032 生效；敏感求职信息需主动告知并确认后才可写入私人分区。
- **检索**：HNSW 索引 + 混合检索（向量 + BM25 + 实体链接）+ 现有 5 因子 rerank。
- 两层连接：会话 observations → 候选事实 → 用户确认或业务证据门禁 → 账本；行为信号只低权重影响短期排序，重复一致后才能提出候选。
- **用户控制与迁移（ADR-0034）**：用户记忆管理页统一处理待确认、纠正和清除；聊天中的明确删除请求进入同一范围确认。闲聊候选 30 天未确认自动清除，关闭默认开启的“自动发现记忆候选”后清除未确认的闲聊候选，但“请记住”仍可单次确认；易变偏好每 90 天复核。旧记忆保守迁移，不将未知来源或低置信旧事实直接激活；旧 admin `delete_memory` 在完整清除可用前拒绝删除请求，归档另行提供。
- 开工先跑对照组基准（Mem0 TS SDK），用现有 eval harness 出基线数值。
- 优先级：画像/账本/检索为核心（P0），观察/反思随 Mastra 引入（P1，不阻塞合并）。

**门禁**：记忆 eval harness 首次记录基线与提升数值；矛盾消解用例（时序变更问题）；检索 hit@k 对比基线；候选确认、敏感信息阻断、纠正与跨层清除读回、权限隔离及旧记忆迁移验证。生产备份目标保留 30 天，发布前核对实际策略；恢复时先重放已完成的清除，再开放记忆读取与提取。
...
| 20 | 记忆写入 | 确定性事件即时写；闲聊信号后台形成待确认候选；两段式决策受 ADR-0034 准入约束 |
...
- ADR-0032～0034：当前偏好优先、定向记忆清除及用户控制与治理政策
