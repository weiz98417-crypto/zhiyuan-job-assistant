...
简历优化 Judge 引擎是整个 CV 优化管线的核心调度器。它接收一个动态优先级模型（有参考简历时 Reference + Operation 并列最高，无参考时 Operation 最高），组装系统提示词，调用 DeepSeek API 生成改写方案，再经由"审阅—确认—保存"流程最终写入持久化存储。
...
┌────────────────────────────────────────────────┐
│ L1  Operation      │ 决定「做什么」             │
│    + Reference     │ （有参考简历时，Reference   │
│    （并列最高）     │   与 Operation 同级最高）    │
│                     │ full | star | quantify |   │
│                     │ keywords                   │
├────────────────────┼────────────────────────────┤
│ L2  JD Filter      │ 决定「重点放哪」             │
├────────────────────┼────────────────────────────┤
...
2. **产品设计决策** — 做了什么选择、为什么、架构如何分层
3. **技术机制细节** — 具体方案、检索机制、记忆机制
4. **闭环反馈** — 如何衡量效果、指标如何驱动迭代
5. **团队协作与推进** — 涉及角色、分期交付、用户培训
6. **量化成果** — 每个维度可衡量的结果

AI 的任务是提取参考简历的思考框架，把同样的深度套到用户的领域中——不是照抄技术方案，而是匹配描述的颗粒度和设计推演逻辑。参考简历勾选需要 Effort ≥ 4。

### 2.3 L3 — Effort（五级改写强度）

Effort 控制所有指令的执行深度，同时直接决定 API 调用的 `temperature` 参数。

| Effort | 名称 | 描述 | Temperature | 量化数据（enablePlaceholders 控制） |
|--------|------|------|-------------|----------------------------------|
| 1 | 温和 | 仅润色措辞，修正语法，改进动词。不添加新内容。 | 0.3 | 无 |
| 2 | 保守 | 优化动词和句式，段落末尾用注释标注可量化点。 | 0.3 | 仅注释提示 |
| 3 | 适中 | 适度调整句式，补充量化描述，微调信息顺序。 | 0.7 | 1-2 数据点 |
| 4 | 大刀 | 大幅重构段落，STAR 格式，大胆推断量化维度。 | 0.9 | 3-4 数据点 |
| 5 | 重写 | 完全重写，所有经历 STAR 格式重组，大量推断量化维度。 | 0.9 | 4+ 数据点 |

enablePlaceholders=true → 量化数据用 `[XX]` 占位符标注；enablePlaceholders=false → 量化数据用自然语言描述。
enablePlaceholders 控制所有 prompt 函数（buildEffortPrompt/buildOperationPrompt/buildReferencePrompt/buildPlaceholderRules），不再有单独函数绕过开关。
...
- Agent 工具调用（无参考简历）→ `fast = true` → Flash（低延迟）
- Agent 工具调用（有参考简历 `referenceIds`）→ `fast = false` → Pro（高质量）
- CV 页面手动优化 → `fast = false` → Pro（始终高质量）
- 所有请求均有 180s 超时保护
