  ...
  return (
    <div className={`markdown-body min-w-0 break-words ${className}`}>
      <ReactMarkdown
        ...
        components={{
          table: ({ children }) => (
            <div className="my-3 w-full overflow-x-auto rounded-[var(--radius-sm)] border border-[var(--color-divider)] bg-[var(--color-surface)]">
              <table className="w-full min-w-max border-collapse text-left text-xs leading-relaxed">
                {children}
              </table>
            </div>
          ),
          ...
          td: ({ children }) => (
            <td className="border border-[var(--color-divider)] px-3 py-2 align-top text-[var(--color-text-soft)]">
              {children}
            </td>
          ),
          ...
          p: ({ children }) => <p className="my-2 leading-relaxed text-[var(--color-text-soft)] first:mt-0 last:mb-0">{children}</p>,
          ...
          li: ({ children }) => <li className="pl-1 leading-relaxed text-[var(--color-text-soft)] marker:text-[var(--color-muted)] [&>p]:my-0">{children}</li>,
          blockquote: ({ children }) => (
          ...
          code: ({ children }) => (
            <code className="rounded bg-[var(--color-bg)] px-1 py-0.5 font-mono text-[0.9em] text-[var(--color-text)]">
              {children}
            </code>
          ),
          pre: ({ children }) => (
            <pre className="my-3 overflow-x-auto rounded-[var(--radius-sm)] border border-[var(--color-divider)] bg-[var(--color-bg)] p-3 text-xs leading-relaxed">
              {children}
            </pre>
          ),
        }}
