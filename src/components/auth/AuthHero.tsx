...
export default function AuthHero({ subtitle }: AuthHeroProps) {
  return (
    <div style={{
        ...
        <div style={{
          position: 'absolute', top: 0, left: 56, bottom: 0, width: 1,
          background: 'oklch(75% 0.04 10 / 0.25)', pointerEvents: 'none', zIndex: 0,
        }} />

        {/* Ink washes */}
        <div style={{
          position: 'absolute', top: -100, right: -60, width: 350, height: 350,
          borderRadius: '50%', pointerEvents: 'none', zIndex: 0,
          background: 'radial-gradient(circle, var(--color-primary-muted) 0%, transparent 70%)',
          opacity: 0.5, animation: 'auth-inkPulse1 8s ease-in-out infinite',
        }} />
        <div style={{
          position: 'absolute', bottom: -80, left: -40, width: 280, height: 280,
          borderRadius: '50%', pointerEvents: 'none', zIndex: 0,
          background: 'radial-gradient(circle, oklch(88% 0.04 85) 0%, transparent 70%)',
          opacity: 0.4, animation: 'auth-inkPulse2 10s ease-in-out infinite',
        }} />
        <div style={{
          position: 'absolute', top: '40%', left: '20%', width: 200, height: 200,
          borderRadius: '50%', pointerEvents: 'none', zIndex: 0,
          background: 'radial-gradient(circle, oklch(82% 0.08 80) 0%, transparent 70%)',
          opacity: 0.25, animation: 'auth-inkPulse3 12s ease-in-out infinite',
        }} />

        {/* Bookmarks — top right */}
        <div style={{
          position: 'absolute', top: -6, right: 80, width: 30, height: 70,
          borderRadius: '0 0 4px 4px', pointerEvents: 'none', zIndex: 2,
          ...
          animation: 'auth-bookmarkSway 6s ease-in-out infinite',
        }} />
        <div style={{
          position: 'absolute', top: -10, right: 48, width: 26, height: 54,
          borderRadius: '0 0 4px 4px', pointerEvents: 'none', zIndex: 2,
        ...
        {[
          ...
          <div key={`kite-${i}`} style={{
            position: 'absolute', ...(k.top ? { top: k.top } : {}),
            ...(k.left ? { left: k.left } : {}), ...(k.right ? { right: k.right } : {}),
            pointerEvents: 'none', zIndex: 3, animation: k.anim,
          }}>
            <KiteSvg w={k.w} h={k.h} op={k.op} />
          </div>
        ))}
        {/* Rising kites */}
        <div style={{
          position: 'absolute', bottom: '6%', right: '3%', pointerEvents: 'none', zIndex: 3,
          animation: 'auth-kiteRise1 9s ease-in-out infinite',
        }}>
          <KiteSvg w={34} h={42} op={0.22} />
        </div>
        <div style={{
          position: 'absolute', bottom: '3%', right: '8%', pointerEvents: 'none', zIndex: 3,
        ...
        {/* ═══ Floating tags — at hero level (full panel width), away from center ═══ */}
        ...
        {/* Central content */}
        <div style={{ position: 'relative', zIndex: 3, textAlign: 'center', maxWidth: 440 }}>
          {/* Animated book icon */}
          ...
          <h1 style={{
            fontFamily: 'var(--font-display)', fontSize: '3.75rem', fontWeight: 700,
            color: 'var(--color-text)', letterSpacing: '0.04em', lineHeight: 1.1,
            marginBottom: 6,
          }}>
          ...
          <p style={{
            fontSize: '1rem', color: 'var(--color-text-soft)', opacity: 0.85,
            position: 'relative', zIndex: 1,
          }}>
            {subtitle}
          </p>
        ...
        {/* Stats footer */}
        <div style={{
          position: 'relative', zIndex: 3, display: 'flex', gap: 40, marginTop: 48,
        }}>
          {[
            ['42', '协作报告'], ['5', '本周活跃'], ['15', '方向覆盖'],
          ].map(([v, l]) => (
            <div key={l} style={{ textAlign: 'center' }}>
              ...
              <div style={{
                fontSize: '0.68rem', color: 'var(--color-muted)', marginTop: 2,
              }}>{l}</div>
            </div>
          ))}
        </div>
...
/* ── Helpers ── */
function tagStyle(o: Record<string, string>): React.CSSProperties {
  return {
    position: 'absolute',
    fontSize: '0.68rem', fontWeight: 500, color: 'var(--color-primary)',
...
function KiteSvg({ w, h, op }: { w: number; h: number; op: number }) {
  return (
    <svg width={w} height={h} viewBox="0 0 48 60" fill="none" style={{ opacity: op }}>
      <polygon points="24,4 42,30 24,56 6,30" fill="none"
      ...
      <path d="M24,56 Q28,62 22,70 Q18,78 24,86" fill="none"
