...
const INTERVIEW_INTENT_PATTERNS = [
  ...
  /帮我.*出.*(题|面试)/,
  /怎么.*(答|说|面)/,
  /(这个|这份|刚才|上面|之前).*(JD|jd|岗位|职位).*(代码|编程|准备|重点|考察|面试)/,
  /(代码|编程).*(考|考察|准备).*(JD|jd|岗位|职位|这个|这份)/,
  /(练习一下|练一练|练一下).*面/,
  /(面经|面试题)/,
  /帮我.*(评分|打分).*(回答|面试)/,
];
...
export const interviewAgent: AgentDefinition = {
  ...
  toolNames: [...INTERVIEW_TOOLS.map(t => t.name), "start_interview_session", "prepare_interview_full", "read_file", "get_reference_detail", "get_recent_jd_context", "search_applications", "get_report_detail"],
