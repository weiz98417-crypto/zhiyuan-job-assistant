...
export interface ReportPdfArtifact extends ExportArtifactRecord {
  reportNum: number;
  company: string;
  role: string;
}

export async function createReportPdfArtifact(
  principal: ExecutionPrincipal,
  reportNum: number,
): Promise<ReportPdfArtifact> {
  const report = await getDataRepositories().reports.get(reportNum, principal.userId);
  ...
  try {
    ...
    await page.setContent(buildReportHtml(report), { waitUntil: "load", timeout: 30_000 });
    await Promise.race([
  ...
  const artifact = await createBinaryExportArtifact(principal, {
    bytes: pdf,
    filename,
    format: "pdf",
...
function buildReportHtml(report: ReportRow): string {
  ...
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8" /><title>${escapeHtml(report.company)} - ${escapeHtml(report.role)} JD评估报告</title><style>
    @page { size: A4; margin: 16mm 14mm; }
    * { box-sizing: border-box; }
    body { margin: 0; color: #1f2937; font-family: "Microsoft YaHei", "PingFang SC", "Noto Sans CJK SC", Arial, sans-serif; font-size: 11px; line-height: 1.55; background: #fff; }
    ...
    .block-title { display: flex; justify-content: space-between; align-items: baseline; border-bottom: 1px solid #111827; margin-bottom: 8px; padding-bottom: 4px; }
