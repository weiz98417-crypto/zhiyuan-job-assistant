...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const row = getDb().prepare("SELECT * FROM sessions WHERE id = ? AND user_id = ?").get(Number(id), user.userId);
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.agentState !== undefined) { sets.push("agent_state_json = ?"); vals.push(JSON.stringify(body.agentState || {})); }
    ...
    getDb().prepare(`UPDATE sessions SET ${sets.join(", ")} WHERE id = ? AND user_id = ?`).run(...vals);
