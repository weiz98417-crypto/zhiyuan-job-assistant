...
- 2026-08-07: The existing public entry point `http://121.43.198.13:38084` was verified through host Nginx, which proxies to Next.js on `127.0.0.1:3100`; the login page and a real superadmin login both returned 200 through that entry point. HTTPS is still not configured, so 8.6 remains open. Do not report the production security rollout as complete until TLS, secure cookies, credential rotation, and SSH hardening are verified.
