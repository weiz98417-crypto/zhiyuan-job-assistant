...
## Data Layer Migration (2026-05-04)

**SQLite (`data/zhiyuan.db`) is now the canonical data store.** User Layer Markdown/TSV files below are **read-only legacy storage**. New writes go to SQLite via `/api/data/*` endpoints or `server-db.ts`.

CLI scripts that previously wrote Markdown/TSV files are deprecated. Use `frontend/src/lib/server-db.ts` or REST API.
